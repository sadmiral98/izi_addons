...
class IZIAnalysisDBPSQL(models.Model):
    _inherit = 'izi.analysis'

    def get_analysis_data_db_psql(self, **kwargs):
        self.ensure_one()
        cursor = self.source_id.get_cursor_db_psql()

        try:
            cursor.execute(kwargs.get('query'))
        except Exception as e:
            raise ValidationError(e)
        res_data = self.source_id.dictfetchall_db_psql(cursor)
        self.source_id.close_cursor_db_psql(cursor)
        return {
            'res_data': res_data,
        }

    def get_field_metric_format_db_psql(self, **kwargs):
        self.ensure_one()
    ...
    def get_field_dimension_format_db_psql(self, **kwargs):
        self.ensure_one()
        query = '%s' % kwargs.get('field_name')
        if not kwargs.get('field_format'):
            return query
    ...
    def get_field_sort_format_db_psql(self, **kwargs):
        self.ensure_one()
    ...
    def get_filter_temp_query_db_psql(self, **kwargs):
        self.ensure_one()
...
class IZIAnalysisFilterOperatorDBPSQL(models.Model):
    _inherit = 'izi.analysis.filter.operator'

    source_type = fields.Selection(
        selection_add=[
            ('db_psql', 'Database PSQL'),
        ])
