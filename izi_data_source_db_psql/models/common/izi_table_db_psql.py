...
class IZITableDBPSQL(models.Model):
    _inherit = 'izi.table'

    def get_table_fields_db_psql(self, **kwargs):
        self.ensure_one()
        ...
        fkey_by_table_column = self.source_id.get_foreignkey_field_db_psql()

        # Get mapping oid and field type FROM pg_type
        typ_by_oid = {}
        cursor.execute("SELECT oid, typname FROM pg_type")
        dict_typs = self.source_id.dictfetchall_db_psql(cursor)
    ...
    def get_table_datas_db_psql(self, **kwargs):
        self.ensure_one()
        cursor = self.source_id.get_cursor_db_psql()
    ...
    def get_data_query_db_psql(self, **kwargs):
        self.ensure_one()
        cursor = self.source_id.get_cursor_db_psql()
    ...
    def get_field_type_origin_db_psql(self, **kwargs):
        self.ensure_one()
