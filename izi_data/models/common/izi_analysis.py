...
class IZIAnalysis(models.Model):
    ...
    analysis_data_script = fields.Text('Analysis Data Script', related='server_action_id.code', readonly=False, default=DEFAULT_DATA_SCRIPT)
    ...
    def copy(self, default=None):
        ...
        res = super(IZIAnalysis, self).copy(default)
        for analysis in res:
            if analysis.method in ('table_view', 'query') and analysis.table_id:
                tables = self.env['izi.table'].search([('name', 'like', analysis.table_id.name)])
                new_identifier = str(len(tables) + 1)
                new_table = analysis.table_id.copy({
                    'name': '%s %s' % (analysis.table_id.name, new_identifier),
                    'db_query': analysis.table_id.db_query,
                })
                new_table.get_table_fields()
                analysis.table_id = new_table.id
                analysis.table_view_id = new_table.id
            elif analysis.method == 'table' and analysis.table_id and analysis.table_id.is_stored:
                tables = self.env['izi.table'].search([('name', 'like', analysis.table_id.name), ('is_stored', '=', True)])
                new_identifier = str(len(tables) + 1)
                if analysis.table_id.cron_id and analysis.table_id.cron_id.ir_actions_server_id:
                    new_code = analysis.table_id.cron_id.ir_actions_server_id.code or ''
                    new_code = new_code.replace(analysis.table_id.store_table_name, '%s_%s' % (analysis.table_id.store_table_name, new_identifier))
                    new_action = analysis.table_id.cron_id.ir_actions_server_id.copy({
                        'name': '%s %s' % (analysis.table_id.cron_id.name, new_identifier),
                        'code': new_code or '',
                    })
                    if new_action:
                        new_cron = analysis.table_id.cron_id.copy({
                            'name': '%s %s' % (analysis.table_id.cron_id.name, new_identifier),
                            'ir_actions_server_id': new_action.id if new_action else False,
                        })
                new_table = analysis.table_id.copy({
                    'name': '%s %s' % (analysis.table_id.name, new_identifier),
                    'cron_id': new_cron.id if new_cron else False,
                })
                for field in analysis.table_id.field_ids:
                    new_field = field.copy({
                        'table_id': new_table.id,
                    })
                    new_table.build_schema_store_table()
                analysis.table_id = new_table.id
            ...
            for metric in self.metric_ids:
                ...
                if field:
                    ...
                    analysis.metric_ids = [(4, new_metric.id)]
            for dimension in self.dimension_ids:
                ...
                if field:
                    ...
                    analysis.dimension_ids = [(4, new_dimension.id)]
            for sort in self.sort_ids:
                ...
                if field:
                    ...
                    analysis.sort_ids = [(4, new_sort.id)]
